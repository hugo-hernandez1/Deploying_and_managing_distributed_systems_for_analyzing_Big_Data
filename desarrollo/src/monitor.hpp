...
protected:
    ...
    /**
     * @brief Ensure the report CSV file has its header written.
     */
    void ensure_report_header();

    /**
     * @brief Append a row to the monitor report CSV.
     */
    void write_report_row(
...
private:
    ...
    std::string topic_name_{"SecureMulticastTopic"};

    //! Path to the CSV report file. Empty disables the CSV report.
    std::string report_csv_path_;

    //! Node identifier (for CSV report attribution in distributed scenarios).
    std::string node_id_;

    //! Handle to the CSV report file.
    std::ofstream report_csv_;

    //! Mutex guarding the report file.
...
#endif /* FASTDDS_STATISTICS_BACKEND_EXAMPLES_CPP_HELLOWORLD_EXAMPLE__MONITOR_H */
